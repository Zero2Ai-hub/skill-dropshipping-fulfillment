...
const { upsertOrder } = require(path.resolve(__dirname, '../../../lib/supabase.js'));
...
async function run() {
  ...
  for (const order of orders) {
    ...
    if (!mapped.matched.length) {
      ...
      // Supabase logging (additive)
      upsertOrder({
        woo_order_id: mapped.orderId,
        status: order.status,
        customer_name: mapped.shippingName,
        customer_email: mapped.shippingEmail,
        total: parseFloat(order.total || 0),
        cj_order_id: null,
        fulfillment_status: `skipped:${reason}`,
      }).catch(() => {});
      appendRejectionLog({
        type: 'order_skipped',
    ...
    try {
      ...
      if (result?.result) {
        ...
        upsertOrder({
          ...
          total: parseFloat(order.total || 0),
          cj_order_id: null,
          fulfillment_status: 'failed:cj_rejected',
        }).catch(() => {});
        appendRejectionLog({
